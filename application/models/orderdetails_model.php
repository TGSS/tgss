...
class Orderdetails_model extends CI_Model {
    ...
    public function get_billing_address_by_order_id($order_id) {
        $query = $this->db->query("SELECT * FROM `billing_addresses` WHERE order_id=" . $order_id);
        return $query->row_array();
    }

    public function get_delivery_address_by_order_id($order_id) {
        $query = $this->db->query("SELECT * FROM `delivery_addresses` WHERE order_id=" . $order_id);
        return $query->row_array();
    }
